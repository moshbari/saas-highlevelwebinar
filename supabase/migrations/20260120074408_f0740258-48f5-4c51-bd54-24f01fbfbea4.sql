-- Create function for daily performance aggregation
-- Uses Dubai timezone (Asia/Dubai) for day boundaries
-- Bypasses the 1000-row limit by aggregating server-side

CREATE OR REPLACE FUNCTION get_daily_performance(
  from_date timestamptz,
  to_date timestamptz,
  webinar_filter uuid DEFAULT NULL
)
RETURNS TABLE (
  day_date date,
